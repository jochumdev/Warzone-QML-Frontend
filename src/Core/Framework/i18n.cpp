...
static bool setLocaleWindows(USHORT usPrimaryLanguage, USHORT usSubLanguage)
{
	...
	if (!success)
	{
		wzLog(LOG_INFO) << QString("Failed to set locale to \"%1\"").arg(usPrimaryLanguage);
	}
	else
	{
		wzLog(LOG_INFO) << QString("Requested locale \"%1\"".arg(usPrimaryLanguage);
	}
...
static bool setLocaleUnix(const char* locale)
{
	...
	{
		wzLog(LOG_INFO) << QString("Failed to set locale to \"%1\"").arg(locale);
	}
	else
	{
		if (strcmp(locale, actualLocale))
		{
			wzLog(LOG_INFO) << QString("Requested locale \"%1\", got \"%2\" instead").arg(locale).arg(actualLocale);
		}
	}
...
bool setLanguage(const char *language)
{
	...
	for (i = 0; i < ARRAY_SIZE(map); i++)
	{
		...
		{
			selectedLanguage = i;
			wzLog(LOG_INFO) << QString("Setting language to \"%1\" (%2)").arg(map[i].name).arg(map[i].language);
...
#  endif
	...
	wzLog(LOG_ERROR) << QString("Requested language \"%1\" not supported.").arg(language);
...
void initI18n(void)
{
	...
	{
		// no system default?
		wzLog(LOG_ERROR) << "initI18n: No system language found";
	}
#if defined(WZ_OS_WIN)
	{
	...
	#ifdef WZ_OS_MAC
	{
		...
		wzLog(LOG_INFO) << QString("resourcePath is %1").arg(resourcePath);
	}
	#else
	textdomainDirectory = bindtextdomain(PACKAGE, LOCALEDIR);
	#endif
	...
	{
		wzLog(LOG_ERROR) << "initI18n: bindtextdomain failed!";
	}
