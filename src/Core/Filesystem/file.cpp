...
PHYSFS_file* openLoadFile(const char* fileName, bool hard_fail)
{
	...
	if (!fileHandle)
	{
		...
		{
			wzLog(LOG_FS) << QString("optional file %1 could not be opened: %2").arg(fileName).arg(PHYSFS_getLastError());
		}
	}
                      ...
                      bool AllocateMem, bool hard_fail)
{
	...
	if (length_read != filesize)
	{
		...
		wzLog(LOG_ERROR) << QString("Reading %1 short: %2").arg(pFileName).arg(PHYSFS_getLastError());
		assert(false);
		return false;
	}
	...
	if (!PHYSFS_close(pfile))
	{
		...
		wzLog(LOG_ERROR) << QString("Error closing %1: %2").arg(pFileName).arg(PHYSFS_getLastError());
		assert(false);
		return false;
	}
...
PHYSFS_file* openSaveFile(const char* fileName)
{
	...
	if (!fileHandle)
	{
		...
		wzLog(LOG_ERROR) << QString("%1 could not be opened: %2").arg(fileName).arg(PHYSFS_getLastError());
		if (found)
		{
...
bool saveFile(const char *pFileName, const char *pFileData, uint32_t fileSize)
{
	...
	{
		wzLog(LOG_ERROR) << QString("%1 could not write: %2").arg(pFileName).arg(PHYSFS_getLastError());
		assert(false);
		return false;
	}
	if (!PHYSFS_close(pfile))
	...
	{
		// weird
		wzLog(LOG_ERROR) << QString("PHYSFS_getRealDir(%1) returns NULL (%2)?!").arg(pFileName).arg(PHYSFS_getLastError());
	}
	else
	{
