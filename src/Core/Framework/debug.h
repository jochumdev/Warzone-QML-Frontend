...
#define ASSERT_FAILURE(expr, expr_string, location_description, function, ...) \
	( \
		(void)WzLog::Logger::Helper(LOG_INFO, __func__).sprintfMe(__VA_ARGS__), \
		(void)WzLog::Logger::Helper(LOG_INFO, function).sprintfMe("Assert in Warzone: %s (%s), last script event: '%s'", \
...
#define info(...) \
do { if (WzLog::Logger::instance().checkLevel(LOG_INFO)) WzLog::Logger::Helper(LOG_INFO, __func__).sprintfMe(__VA_ARGS__); } while(0)
 ...
 */
#define debug(level, ...) \
do { if (WzLog::Logger::instance().checkLevel(level)) WzLog::Logger::Helper(level, __func__).sprintfMe(__VA_ARGS__); } while(0)
